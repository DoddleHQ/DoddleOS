...
```bash
/cro:form "Optimize Doddle Marketing OS's demo request form: Name, Email, Company, Phone, Title, Team Size, Message"
```
...
```bash
/cro:signup "Analyze Doddle Marketing OS's trial signup: Email → Password → Company → Team Size → Use Case → Payment"
```
...
Audit Doddle Marketing OS's current demo request form:
...
Design an optimized signup flow for Doddle Marketing OS:

```bash
/cro:signup "Design optimal trial signup for Doddle Marketing OS. Current: 5-step process. Goal: maximize trial starts."
```
...
```bash
/cro:form "Design 3-step progressive form for Doddle Marketing OS enterprise demo"
```
...
3. **Create A/B test:**
   ```bash
   /test:ab-setup "Test 7-field vs 4-field form for Doddle Marketing OS demos"
   ```
