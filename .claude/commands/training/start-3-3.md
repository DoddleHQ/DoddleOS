...
```bash
/cro:popup "Design exit-intent popup for Doddle Marketing OS blog. Goal: capture emails for 'Remote Team Productivity Guide' lead magnet."
```
...
For Doddle Marketing OS: "When a team member sees their team's focus schedule and blocks distraction-free time"
...
Use `/cro:onboarding` to optimize Doddle Marketing OS's activation:

```bash
/cro:onboarding "Design onboarding for Doddle Marketing OS. Aha moment: seeing team focus schedule. Current activation: 15% of trials. Goal: 40%."
```
...
```bash
/cro:paywall "Design upgrade screen for Doddle Marketing OS. Trigger: user hits 5-user limit on free tier. Goal: convert to Team plan ($12/user)."
```
...
Complete these exercises for Doddle Marketing OS:

### 1. Exit Intent Popup
```bash
/cro:popup "Exit intent for Doddle Marketing OS pricing page - capture leads who leave without trial"
```
